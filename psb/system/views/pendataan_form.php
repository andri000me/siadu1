...
else { $ttl='Tambah'; // Nilai field default
	$data=Array();
	$data['replid']=0;
	$data['kriteria']=$krit;
	$data['golongan']=$gol;
...
$xform->table_begin();
	...
	$xform->group_begin('Angsuran');
		...
		if($opt=='af') $cicilpb=$data['jmlangsur']>0?$biaya/$data['jmlangsur']:0;
		else $cicilpb=$data['angsuran'];
	...
	$xform->group_begin('Discount');
		...
		$xform->fi('Discount tunai',iSelect('disctunai',$discount,$data['discount'],'width:200px','pendataan_countnet()'));
